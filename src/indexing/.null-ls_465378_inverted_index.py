...
PACKET_HEADER = bytes.fromhex("756E6A")
PACKET_FOOTER = bytes.fromhex("6B656C6172")
MAX_PACKET_LEN = 4088
...
def trimHeader(b: bytes) -> bytes:
    return b[len(PACKET_HEADER):]


def trimFooter(b: bytes) -> bytes:
    return b[:-len(PACKET_FOOTER)]
